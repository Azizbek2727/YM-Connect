    ...
    ($name:ident, $variant:ident, $doc:literal) => {
        ...
        impl $name {
            ...
            pub fn new(value: impl Into<String>) -> Result<Self, PairingModelError> {
                ...
                if value.is_empty() {
                    return Err(PairingModelError::$variant);
                }
...
identifier!(
    PairingId,
    EmptyPairingId,
    "Validated pairing-session identifier."
);
identifier!(
    ChallengeId,
    EmptyChallengeId,
    "Validated pairing-challenge identifier."
);
identifier!(
    BridgeId,
    EmptyBridgeId,
    "Validated Bridge identity identifier."
);
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidPublicKeyLength { actual } => {
                write!(f, "public key must contain 32 bytes, got {actual}")
            }
            Self::InvalidNonceLength { actual } => {
                write!(f, "challenge nonce must contain 32 bytes, got {actual}")
            }
            Self::InvalidConfirmationTagLength { actual } => {
                write!(f, "confirmation tag must contain 16 bytes, got {actual}")
            }
...
impl PairingPublicKey {
    ...
    pub fn new(bytes: impl AsRef<[u8]>) -> Result<Self, PairingModelError> {
        ...
        Ok(Self(Arc::new(<[u8; 32]>::try_from(bytes).map_err(
            |_| PairingModelError::InvalidPublicKeyLength {
                actual: bytes.len(),
            },
        )?)))
    }
...
impl PairingNonce {
    ...
    pub fn new(bytes: impl AsRef<[u8]>) -> Result<Self, PairingModelError> {
        let bytes = bytes.as_ref();
        Ok(Self(Arc::new(<[u8; 32]>::try_from(bytes).map_err(
            |_| PairingModelError::InvalidNonceLength {
                actual: bytes.len(),
            },
        )?)))
    }
...
impl PairingConfirmationTag {
    ...
    pub fn new(bytes: impl AsRef<[u8]>) -> Result<Self, PairingModelError> {
        let bytes = bytes.as_ref();
        Ok(Self(Arc::new(<[u8; 16]>::try_from(bytes).map_err(
            |_| PairingModelError::InvalidConfirmationTagLength {
                actual: bytes.len(),
            },
        )?)))
    }
    ...
    pub fn new(mut value: CapabilitySet) -> Result<Self, PairingModelError> {
        ...
        let invalid = value.supported.windows(2).any(|v| v[0] == v[1])
            ...
            || value
                .supported
                .iter()
                .any(|v| Capability::try_from(*v).is_err() || *v == 0)
            || value
                .required
                .iter()
                .any(|v| Capability::try_from(*v).is_err() || *v == 0)
            || value
                .required
                .iter()
                .any(|v| value.supported.binary_search(v).is_err());
        if invalid {
...
impl PairingChallenge {
    ...
    /// Returns identifier.
    #[must_use]
    pub const fn id(&self) -> &ChallengeId {
        &self.id
    }
    /// Returns nonce.
    #[must_use]
    ...
    #[must_use]
    pub const fn bridge_ephemeral_key(&self) -> &PairingPublicKey {
        &self.bridge_ephemeral_key
    }
    /// Returns creation time.
    #[must_use]
    pub const fn created_at(&self) -> PairingTimestamp {
        self.created_at
    }
    /// Returns expiry time.
    #[must_use]
    pub const fn expires_at(&self) -> PairingTimestamp {
        self.expires_at
    }
    /// Returns expiry status.
    #[must_use]
    pub fn is_expired(&self, observed_at: PairingTimestamp) -> bool {
...
impl PairingRequest {
    ...
    /// Returns X25519 key.
    #[must_use]
    pub const fn ephemeral_key(&self) -> &PairingPublicKey {
        &self.ephemeral_key
    }
    /// Returns protocol version.
    #[must_use]
...
impl PairingResponse {
    ...
    #[must_use]
    pub const fn confirmation_tag(&self) -> &PairingConfirmationTag {
...
impl PairingState {
    ...
    pub const fn is_terminal(self) -> bool {
        matches!(
            self,
            Self::Completed | Self::Rejected | Self::Expired | Self::Revoked | Self::Cancelled
        )
    }
        ...
        matches!(
            ...
            (Self::Idle, Self::ChallengeCreated | Self::Cancelled)
                | (
                    ...
                    Self::ChallengeSent | Self::Expired | Self::Cancelled
                )
                | (
                    Self::ChallengeSent,
                    Self::ResponseReceived | Self::Rejected | Self::Expired | Self::Cancelled
                )
                | (
                    Self::ResponseReceived,
                    Self::IdentityVerified | Self::Rejected | Self::Expired | Self::Cancelled
                )
                | (
                    Self::IdentityVerified,
                    Self::TrustEstablished | Self::Rejected | Self::Revoked | Self::Cancelled
                )
...
impl TrustMetadataKey {
    ...
    pub fn new(value: impl Into<String>) -> Result<Self, PairingModelError> {
        let value = value.into();
        if value.is_empty() {
            return Err(PairingModelError::EmptyMetadataKey);
        }
        Ok(Self(Arc::from(value)))
    }
    /// Returns text.
...
impl TrustMetadata {
    ...
    pub fn insert(
        &mut self,
        key: TrustMetadataKey,
        value: TrustMetadataValue,
    ) -> Option<TrustMetadataValue> {
...
impl TrustedPeer {
    ...
    #[must_use]
    pub const fn bridge_identity(&self) -> &BridgeIdentity {
    ...
    /// Returns peer key.
    #[must_use]
    pub const fn peer_identity_key(&self) -> &PairingPublicKey {
    ...
    /// Returns trust time.
    #[must_use]
    pub const fn trusted_at(&self) -> PairingTimestamp {
        self.trusted_at
    }
    /// Returns last verification time.
    #[must_use]
    pub const fn last_verified_at(&self) -> PairingTimestamp {
        self.last_verified_at
    }
    /// Returns revocation time.
    #[must_use]
    pub const fn revoked_at(&self) -> Option<PairingTimestamp> {
        self.revoked_at
    }
    /// Returns metadata.
    #[must_use]
    pub const fn metadata(&self) -> &TrustMetadata {
        &self.metadata
    }
    /// Returns revision.
    #[must_use]
...
impl PairingSession {
    ...
    pub const fn new(
        ...
        Self {
            ...
            revision: PairingRevision::INITIAL,
            created_at,
    ...
    /// Returns response.
    #[must_use]
    pub const fn response(&self) -> Option<&PairingResponse> {
        self.response.as_ref()
    }
    /// Returns request.
    #[must_use]
    pub fn request(&self) -> Option<&PairingRequest> {
        self.response.as_ref().map(PairingResponse::request)
    }
    /// Returns state.
    #[must_use]
    ...
    #[must_use]
    pub const fn challenge_consumed(&self) -> bool {
        self.challenge_consumed
    }
    /// Returns revision.
    #[must_use]
    pub const fn revision(&self) -> PairingRevision {
    ...
    /// Returns update time.
    #[must_use]
    pub const fn updated_at(&self) -> PairingTimestamp {
        self.updated_at
    }
    pub(crate) fn next(
        &self,
        ...
        if let Some(value) = challenge {
            next.challenge = Some(value);
        }
        if let Some(value) = response {
            next.response = Some(value);
        }
