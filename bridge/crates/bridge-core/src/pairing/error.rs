...
impl PairingError {
    ...
    fn format_pairing_session(&self, f: &mut fmt::Formatter<'_>) -> Option<fmt::Result> {
        match self {
            Self::PairingNotFound { pairing_id } => {
                Some(write!(f, "pairing session {pairing_id} does not exist"))
            }
            Self::DuplicatePairing { pairing_id } => {
                Some(write!(f, "pairing session {pairing_id} already exists"))
            }
            Self::DuplicateChallenge { challenge_id } => {
                Some(write!(f, "pairing challenge {challenge_id} already exists"))
            }
            Self::InvalidChallengeLifetime { pairing_id } => Some(write!(
                f,
            ...
            Self::InvalidTransition {
                ...
                f,
                "pairing session {pairing_id} cannot transition from {previous:?} to {requested:?}"
            )),
            Self::TerminalPairing { pairing_id, state } => Some(write!(
                f,
                "pairing session {pairing_id} is terminal in state {state:?}"
            )),
            Self::StaleRevision {
                pairing_id,
                expected,
                ...
                f,
                "pairing session {pairing_id} revision is stale: expected {}, actual {}",
                expected.get(),
                actual.get()
            )),
            Self::TimestampRegression {
                ...
                f,
                "pairing session {pairing_id} timestamp regressed from {} to {}",
            ...
            Self::ChallengeExpired {
                ...
                f,
                "challenge {challenge_id} for pairing {pairing_id} expired"
            )),
            Self::ChallengeNotExpired {
                ...
                f,
                "challenge {challenge_id} for pairing {pairing_id} has not expired"
            )),
            Self::ReplayDetected {
                pairing_id,
                challenge_id,
                ...
                f,
                "challenge {challenge_id} for pairing {pairing_id} was replayed"
            )),
            Self::ChallengeMismatch {
                ...
                f,
                "pairing {pairing_id} expected challenge {expected}, got {actual}"
            )),
            _ => None,
        }
    }

    fn format_trust(&self, f: &mut fmt::Formatter<'_>) -> Option<fmt::Result> {
        match self {
            Self::DuplicateDeviceIdentity { device_id } => Some(write!(
                f,
                "device {device_id} is already trusted under another identity"
            )),
            Self::DuplicateIdentityKey { existing_device_id } => Some(write!(
                f,
            ...
            Self::TrustReplacementForbidden { device_id } => Some(write!(
                f,
                "trust replacement for device {device_id} is forbidden"
            )),
            Self::TrustNotFound { device_id } => {
                Some(write!(f, "trusted peer {device_id} does not exist"))
            }
            Self::StaleTrustRevision {
                ...
                f,
                "trusted peer {device_id} revision is stale: expected {}, actual {}",
                expected.get(),
                actual.get()
            )),
            Self::TrustTimestampRegression {
                ...
                f,
                "trusted peer {device_id} timestamp regressed from {} to {}",
                previous.as_unix_millis(),
                requested.as_unix_millis()
            )),
            _ => None,
...
impl fmt::Display for PairingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(result) = self.format_pairing_session(f) {
            return result;
        }
        if let Some(result) = self.format_trust(f) {
            return result;
        }
        ...
        match self {
            ...
            Self::InvalidPublicKey { code, message } => {
                write!(f, "public key rejected ({code}): {message}")
            }
            Self::InvalidSignature { code, message } => {
                write!(f, "Ed25519 signature rejected ({code}): {message}")
            }
            Self::InvalidKeyConfirmation { code, message } => {
                write!(f, "pairing confirmation rejected ({code}): {message}")
            }
            ...
            Self::MissingRequiredCapabilities => {
                f.write_str("required pairing capabilities are missing")
            }
            Self::RevisionExhausted => f.write_str("pairing or trust revision is exhausted"),
            Self::StateInvariant { message } => {
                write!(f, "Pairing Core state invariant failed: {message}")
            }
            Self::PairingNotFound { .. }
            | Self::DuplicatePairing { .. }
            | Self::DuplicateChallenge { .. }
            | Self::InvalidChallengeLifetime { .. }
            | Self::MissingSession { .. }
            | Self::InvalidTransition { .. }
            | Self::TerminalPairing { .. }
            | Self::StaleRevision { .. }
            | Self::TimestampRegression { .. }
            | Self::ChallengeExpired { .. }
            | Self::ChallengeNotExpired { .. }
...
impl From<RegistryStateError> for PairingError {
    fn from(source: RegistryStateError) -> Self {
        Self::State(StateError::from(source))
    }
}
