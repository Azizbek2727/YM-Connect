...
mod tests {
    ...
    const ED25519_SIGNATURE: [u8; 64] = [
        0x97, 0xe6, 0xdd, 0x35, 0xe8, 0xd0, 0x04, 0xef, 0x54, 0x44, 0x53, 0x68, 0x30, 0xef,
        0xcc, 0xbc, 0x37, 0x67, 0x93, 0xf8, 0xc1, 0x66, 0xc3, 0xe9, 0x17, 0x5f, 0xe3, 0x61,
        0x14, 0x02, 0x89, 0x95, 0x1e, 0x6e, 0x23, 0x04, 0x59, 0xba, 0x85, 0x51, 0x37, 0xf3,
        0x8b, 0x58, 0xfc, 0xc2, 0x1e, 0xed, 0xa2, 0x31, 0x03, 0xfd, 0x9a, 0xb9, 0x36, 0x74,
        0x3c, 0x60, 0x8f, 0xab, 0xee, 0xe3, 0x41, 0x06,
    ];
    ...
    const PEER_PUBLIC: [u8; 32] = [
        0x79, 0xa6, 0x31, 0xee, 0xde, 0x1b, 0xf9, 0xc9, 0x8f, 0x12, 0x03, 0x2c, 0xde, 0xad,
        0xd0, 0xe7, 0xa0, 0x79, 0x39, 0x8f, 0xc7, 0x86, 0xb8, 0x8c, 0xc8, 0x46, 0xec, 0x89,
        0xaf, 0x85, 0xa5, 0x1a,
    ];
    const CONFIRMATION_TAG: [u8; 16] = [
        0x30, 0x5d, 0x1a, 0xde, 0xb1, 0x70, 0x4b, 0x95, 0x13, 0x3e, 0xf1, 0x7a, 0x41, 0x06,
        0xef, 0xd9,
    ];
    ...
    impl DeterministicSecretSource {
        fn new(entries: impl IntoIterator<Item = (PairingPublicKey, [u8; 32])>) -> Self {
            Self { secrets: Mutex::new(entries.into_iter().collect()) }
        }
    }
    ...
    fn ed25519_known_answer_vector_is_strict() -> TestResult {
        let provider = RustCryptoPairingProvider::new(Arc::new(
            DeterministicSecretSource::new([]),
        ));
