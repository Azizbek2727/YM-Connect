...
impl<K> RegistryDelta<K> {
    ...
    #[must_use]
    pub fn inserted(&self) -> &[K] { &self.inserted }
    /// Returns replaced keys in deterministic order.
    #[must_use]
    pub fn replaced(&self) -> &[K] { &self.replaced }
    /// Returns removed keys in deterministic order.
    #[must_use]
    pub fn removed(&self) -> &[K] { &self.removed }
    ...
    K: Clone,
{
        ...
        V: StateRegistryValue<Key = K>,
    {
        ...
        for key in before.keys() {
            match after.get(key) {
                Some(after_value) if before.get(key) != Some(after_value) => replaced.push(key.clone()),
                Some(_) => {}
                None => removed.push(key.clone()),
            }
        }
        for key in after.keys() {
            if !before.contains_key(key) { inserted.push(key.clone()); }
        }
...
impl BridgeStateEvent {
    pub(super) fn between(
        ...
        if before.lifecycle != after.lifecycle {
            changes.push(BridgeStateChange::Lifecycle { previous: before.lifecycle, current: after.lifecycle });
        }
        if before.configuration != after.configuration { changes.push(BridgeStateChange::Configuration); }
        ...
        changes.extend(session_transitions.into_iter().map(BridgeStateChange::SessionLifecycle));
        changes.extend(transport_events_between(before, after).into_iter().map(BridgeStateChange::Transport));
        changes.extend(pairing_events_between(before, after).into_iter().map(BridgeStateChange::Pairing));

        let sessions = RegistryDelta::between(&before.sessions, &after.sessions);
        if !sessions.is_empty() { changes.push(BridgeStateChange::Sessions(sessions)); }
        let devices = RegistryDelta::between(&before.devices, &after.devices);
        if !devices.is_empty() { changes.push(BridgeStateChange::Devices(devices)); }
        let connectors = RegistryDelta::between(&before.connectors, &after.connectors);
        if !connectors.is_empty() { changes.push(BridgeStateChange::Connectors(connectors)); }
        let connections = RegistryDelta::between(&before.connections, &after.connections);
        if !connections.is_empty() { changes.push(BridgeStateChange::Connections(connections)); }
        let pairing_sessions = RegistryDelta::between(&before.pairing_sessions, &after.pairing_sessions);
        if !pairing_sessions.is_empty() { changes.push(BridgeStateChange::PairingSessions(pairing_sessions)); }
        let trusted_peers = RegistryDelta::between(&before.trusted_peers, &after.trusted_peers);
        if !trusted_peers.is_empty() { changes.push(BridgeStateChange::TrustedPeers(trusted_peers)); }
        let capabilities = RegistryDelta::between(&before.capabilities, &after.capabilities);
        if !capabilities.is_empty() { changes.push(BridgeStateChange::Capabilities(capabilities)); }

        Self { previous_revision, revision, changes: changes.into(), snapshot }
    }

    /// Returns the revision observed before the commit.
    #[must_use]
    pub const fn previous_revision(&self) -> StateRevision { self.previous_revision }
    /// Returns the committed revision.
    #[must_use]
    pub const fn revision(&self) -> StateRevision { self.revision }
    /// Returns changes in deterministic subsystem order.
    #[must_use]
    pub fn changes(&self) -> &[BridgeStateChange] { &self.changes }
    /// Returns the immutable snapshot produced by the commit.
    #[must_use]
    pub const fn snapshot(&self) -> &BridgeStateSnapshot { &self.snapshot }
}

fn transport_events_between(before: &BridgeStateData, after: &BridgeStateData) -> Vec<TransportEvent> {
    let mut events = Vec::new();
    for (connection_id, connection) in after.connections.iter() {
        match before.connections.get(connection_id) {
            None => events.push(TransportEvent::lifecycle(connection_id.clone(), connection.transport_id().clone(), None, Some(connection.state()), connection.revision(), connection.updated_at())),
            Some(previous) => {
                if previous.state() != connection.state() {
                    events.push(TransportEvent::lifecycle(connection_id.clone(), connection.transport_id().clone(), Some(previous.state()), Some(connection.state()), connection.revision(), connection.updated_at()));
                }
                if previous.session_id() != connection.session_id() {
                    events.push(TransportEvent::session_binding(connection_id.clone(), connection.transport_id().clone(), previous.session_id().cloned(), connection.session_id().cloned(), connection.revision(), connection.updated_at()));
                }
            }
        }
    }
...
fn pairing_events_between(before: &BridgeStateData, after: &BridgeStateData) -> Vec<PairingEvent> {
    ...
    for (pairing_id, session) in after.pairing_sessions.iter() {
        match before.pairing_sessions.get(pairing_id) {
            None => events.push(PairingEvent::lifecycle(pairing_id.clone(), None, Some(session.state()), session.revision(), session.updated_at())),
            Some(previous) if previous.state() != session.state() => events.push(PairingEvent::lifecycle(pairing_id.clone(), Some(previous.state()), Some(session.state()), session.revision(), session.updated_at())),
            Some(_) => {}
        }
    }
    for (device_id, peer) in after.trusted_peers.iter() {
        match before.trusted_peers.get(device_id) {
            None => events.push(PairingEvent::trust(device_id.clone(), None, Some(peer.is_revoked()), peer.revision(), peer.last_verified_at())),
            Some(previous) if previous != peer => events.push(PairingEvent::trust(device_id.clone(), Some(previous.is_revoked()), Some(peer.is_revoked()), peer.revision(), peer.last_verified_at())),
            Some(_) => {}
        }
    }
    events.sort_by(|left, right| {
