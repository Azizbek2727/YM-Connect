...
impl fmt::Display for RegistryKind {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            ...
            Self::PairingResponses => "pairing responses",
...
impl RegistryStateError {
    fn invalid_identifier(
        ...
        Self { registry, operation, failure: RegistryFailure::InvalidIdentifier, key: None, identifier: Some(identifier) }
    }
    ...
    fn key_failure(
        ...
        Self { registry, operation, failure, key: Some(key.to_string()), identifier: None }
    }

    /// Returns the affected registry.
    #[must_use]
    pub const fn registry(&self) -> RegistryKind { self.registry }
    /// Returns the failed operation.
    #[must_use]
    pub const fn operation(&self) -> RegistryOperation { self.operation }
    /// Returns the structured failure reason.
    #[must_use]
    pub const fn failure(&self) -> RegistryFailure { self.failure }
...
impl fmt::Display for RegistryStateError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match (&self.key, &self.identifier, self.failure) {
            (_, Some(identifier), RegistryFailure::InvalidIdentifier) => write!(formatter, "cannot {} {} registry entry: {identifier}", self.operation, self.registry),
            (Some(key), _, RegistryFailure::DuplicateKey) => write!(formatter, "cannot {} {} registry entry {key:?}: key already exists", self.operation, self.registry),
            (Some(key), _, RegistryFailure::MissingKey) => write!(formatter, "cannot {} {} registry entry {key:?}: key does not exist", self.operation, self.registry),
            _ => write!(formatter, "cannot {} {} registry entry", self.operation, self.registry),
        }
    }
}
...
impl Error for RegistryStateError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        self.identifier.as_ref().map(|source| source as &(dyn Error + 'static))
    }
}
...
impl<K> RegistryMutation<K> {
    ...
    #[must_use]
    pub const fn changed(&self) -> bool { !matches!(self, Self::Unchanged(_)) }
}
...
#[derive(Clone, Copy)]
enum WriteOperation { Insert, Replace, Upsert }
    ...
    V: StateRegistryValue,
{
    ...
    #[must_use]
    pub fn contains_key(&self, key: &V::Key) -> bool { self.entries.contains_key(key) }
    /// Returns a registered value.
    #[must_use]
    pub fn get(&self, key: &V::Key) -> Option<&V> { self.entries.get(key).map(AsRef::as_ref) }
    /// Returns a shared registered value.
    #[must_use]
    pub fn get_shared(&self, key: &V::Key) -> Option<Arc<V>> { self.entries.get(key).cloned() }
    ...
    #[must_use]
    pub fn keys(&self) -> impl DoubleEndedIterator<Item = &V::Key> + ExactSizeIterator { self.entries.keys() }
    ...
    pub fn remove(&mut self, key: &V::Key) -> Result<(RegistryMutation<V::Key>, Arc<V>), RegistryStateError> {
        self.entries.remove(key).map_or_else(
            || Err(RegistryStateError::key_failure(V::REGISTRY_KIND, RegistryOperation::Remove, RegistryFailure::MissingKey, key)),
            |value| Ok((RegistryMutation::Removed(key.clone()), value)),
        )
    }
    /// Removes a value when present.
    #[must_use]
    pub fn remove_if_present(&mut self, key: &V::Key) -> Option<Arc<V>> { self.entries.remove(key) }
    /// Removes all values and returns the number removed.
    #[must_use]
    pub fn clear(&mut self) -> usize { let removed = self.entries.len(); self.entries.clear(); removed }

    fn write(&mut self, value: V, operation: WriteOperation) -> Result<RegistryMutation<V::Key>, RegistryStateError> {
        let public_operation = operation.public();
        let key = value.registry_key().map_err(|identifier| RegistryStateError::invalid_identifier(V::REGISTRY_KIND, public_operation, identifier))?;
        let existing = self.entries.get(&key);
        if matches!(operation, WriteOperation::Insert) && existing.is_some() {
            return Err(RegistryStateError::key_failure(V::REGISTRY_KIND, public_operation, RegistryFailure::DuplicateKey, &key));
        }
        if matches!(operation, WriteOperation::Replace) && existing.is_none() {
            return Err(RegistryStateError::key_failure(V::REGISTRY_KIND, public_operation, RegistryFailure::MissingKey, &key));
        }
        if existing.is_some_and(|existing| existing.as_ref() == &value) {
            return Ok(RegistryMutation::Unchanged(key));
        }
        let mutation = if existing.is_some() { RegistryMutation::Replaced(key.clone()) } else { RegistryMutation::Inserted(key.clone()) };
...
impl StateRegistryValue for SessionEstablished {
    ...
    fn registry_key(&self) -> Result<Self::Key, StateIdentifierError> { SessionId::new(self.session_id.clone()) }
}

impl StateRegistryValue for DeviceDescriptor {
    type Key = DeviceId;
    const REGISTRY_KIND: RegistryKind = RegistryKind::Devices;
    fn registry_key(&self) -> Result<Self::Key, StateIdentifierError> { DeviceId::new(self.device_id.clone()) }
}
...
impl StateRegistryValue for BrowserDescriptor {
    ...
    fn registry_key(&self) -> Result<Self::Key, StateIdentifierError> { ConnectorId::new(self.connector_id.clone()) }
}
...
impl CapabilityRegistration {
    ...
    #[must_use]
    pub fn new(owner: CapabilityOwner, capabilities: CapabilitySet) -> Self { Self { owner, capabilities } }
    /// Returns the capability owner.
    #[must_use]
    pub const fn owner(&self) -> &CapabilityOwner { &self.owner }
    /// Returns the canonical capability set.
    #[must_use]
    pub const fn capabilities(&self) -> &CapabilitySet { &self.capabilities }
}
...
impl StateRegistryValue for CapabilityRegistration {
    ...
    fn registry_key(&self) -> Result<Self::Key, StateIdentifierError> { Ok(self.owner.clone()) }
}
